...
@Component({
  ...
  template: `
    <aside class="w-64 flex-shrink-0 bg-slate-800 text-slate-200 flex flex-col">
      <div class="h-16 flex items-center justify-center border-b border-slate-700">
        <h1 class="text-2xl font-bold text-white tracking-wider">Advika<span class="text-indigo-400">POS</span></h1>
      </div>
      <nav class="flex-1 px-4 py-6 space-y-2">
        @for (item of navItems(); track item.path) {
          <a [routerLink]="item.path" routerLinkActive="bg-slate-700 text-white" 
             class="flex items-center px-4 py-2.5 rounded-lg hover:bg-slate-700 transition-colors duration-200">
             <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" [innerHTML]="sanitizer.bypassSecurityTrustHtml(item.icon)"></svg>
            <span>{{ item.name }}</span>
          </a>
        }
      </nav>
      <div class="px-4 py-4 border-t border-slate-700">
        <div class="flex items-center">
            <img class="h-10 w-10 rounded-full object-cover" src="https://picsum.photos/seed/user/100/100" alt="User avatar">
            <div class="ml-3">
                <p class="text-sm font-medium text-white">Aditi Sharma</p>
  ...
  host: {
    class: 'fixed inset-y-0 left-0 z-30 md:relative md:translate-x-0'
  }
})
